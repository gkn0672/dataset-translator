...
CACHE_FAIL_PROMPT = {}
MAX_LIST_RETRIES = 6  # The maximum number of retries for vllm list translation
MAX_STRING_RETRIES = 3  # The maximum number of retries for vllm string translation
...
class VLLMEngine(BaseEngine):
    """
    Engine implementation for VLLM using the VLLM API server.
    """

    def __init__(
        self, api_url="http://localhost:8000/v1/chat/completions", model_name="llama3"
    ):
        """
        Initialize the VLLM Engine using API.

        Args:
            api_url (str): URL of the VLLM API server
            model_name (str): The name of the model to use in API requests
        """
        super().__init__()
    ...
    def construct_schema_prompt(schema: dict) -> str:
        """
        Constructs a schema prompt for JSON responses.
    ...
    def _do_translate(
        ...
        **kwargs,
        ...
        """
        Perform translation using VLLM API server.
        ...
        if data_type == "list":
            ...
            user_content = (
                f"{prefix_prompt}\n\n{prompt}\n\n{postfix_prompt_text}\n\n"
                f"Translate the provided text from **English** to **Vietnamese**, "
                f"considering the context. DO NOT add extra information or remove any information inside the fields. "
                f"Return the translated results in the respective fields of the JSON object."
            )

            # Format as chat messages for the API
            messages = [
                {
                    "role": "system",
            ...
            total_tokens = len(
                (system_prompt + postfix_system_prompt + user_content).split()
            )
            ...
            try:
                # Prepare payload for VLLM API (OpenAI-compatible API)
                payload = {
                ...
                if input_hash in CACHE_FAIL_PROMPT:
                    if CACHE_FAIL_PROMPT[input_hash] >= MAX_LIST_RETRIES:
                        print(
                            f"\nUnavoidable exception: {e}\nVLLM max retries reached for list translation"
                        )
                        return [fail_translation_code] * len(input_data)
                    else:
                ...
                print(f"\nCurrent vllm fail cache: {CACHE_FAIL_PROMPT}\n")
                raise e
            ...
            user_content = (
                ...
                f"{input_data}\n\n"
                f"{postfix_separator}\n{postfix_prompt_block}\n\n"
                f"Translate all the above text inside the translation block from **English** to **Vietnamese**. "
                f"DO NOT add extra information or remove any information inside, just translate."
            )
            ...
            try:
                ...
                if input_hash in CACHE_FAIL_PROMPT:
                    if CACHE_FAIL_PROMPT[input_hash] >= MAX_STRING_RETRIES:
                        print(
                            f"\nUnavoidable exception: {e}\nVLLM max retries reached for string translation"
                        )
                        return fail_translation_code
                    else:
                        CACHE_FAIL_PROMPT[input_hash] += 1
