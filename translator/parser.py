...
class DataParser(metaclass=ForceBaseCallMeta):
    def __init__(
        ...
        translator: BaseEngine = GroqEngine,
        source_lang: str = "en",
        target_lang: str = "vi",
    ...
    def __translate_per_key(
        self, example: Dict, translator: BaseEngine = None, progress_idx: int = 0
    ) -> Dict:
        """
    ...
    def __translate_texts(
        ...
        if isinstance(src_texts, list):
            if len(src_texts) == 1:
                ...
                if self.verbose:
                    safe_tqdm_write(
                        "List contain only one element, extract the element and translate..."
                    )
            if len(src_texts) == 0:
                if self.verbose:
                    safe_tqdm_write("Empty list, skipping...")
                return src_texts
        else:
            if len(src_texts) == 0:
                if self.verbose:
                    safe_tqdm_write("Empty string, skipping...")
                return src_texts
    ...
    def translate_converted(
        ...
        if len(converted_data) > self.large_chunks_threshold and large_chunk is None:
            ...
            for idx, large_chunk in enumerate(
                tqdm(large_chunks, desc="Translating large chunk ", colour="red")
        ...
        for example in tqdm(
            ...
            translated_data_example = self.__translate_per_key(
                example,
                translator,
                progress_idx=int(re.findall(r"\d+", desc)[0])
    ...
    def save(self) -> None:
        ...
        if self.do_translate:
            ...
            assert self.converted_data_translated is not None, (
